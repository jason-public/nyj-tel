  ...
  return (
    <div className="border border-slate-100 dark:border-slate-700 rounded-lg overflow-hidden">
      <button
        ...
        className="w-full flex items-center gap-2 p-3 bg-slate-50 dark:bg-slate-800/40 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors text-left"
      >
        {isExpanded ? <ChevronDown className="w-4 h-4 text-slate-500 dark:text-slate-400 shrink-0" /> : <ChevronRight className="w-4 h-4 text-slate-500 dark:text-slate-400 shrink-0" />}
        <Folder className="w-4 h-4 text-teal-600 shrink-0" />
        <span className="font-bold text-slate-800 dark:text-slate-100">{orgName}</span>
        <span className="text-xs font-medium text-slate-500 dark:text-slate-400 bg-slate-200 dark:bg-slate-700 px-2 py-0.5 rounded-full ml-auto">
          {depts.length}
        </span>
      </button>
      <AnimatePresence initial={false}>
        {isExpanded && (
          ...
          >
            <div className="p-2 pl-6 md:pl-8 space-y-1 bg-white dark:bg-slate-800 border-t border-slate-100 dark:border-slate-700">
              {depts.map((dept) => (
...
function DeptNode({ dept, hasSearch, onDeptSelect }: { dept: Department; hasSearch: boolean; onDeptSelect: (dept: Department) => void }) {
  ...
  return (
    <div>
      <div className="flex items-center gap-2 py-1.5 hover:bg-slate-50 dark:bg-slate-800/50 rounded px-2 -mx-2 transition-colors group">
        {hasChildren ? (
          <button onClick={() => setIsExpanded(!isExpanded)} className="p-0.5 rounded hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors">
        ...
        <button onClick={() => onDeptSelect(dept)} className="font-medium text-slate-700 dark:text-slate-300 hover:text-teal-600 hover:underline text-left">
          {dept.deptName}
        </button>
      </div>
      <AnimatePresence initial={false}>

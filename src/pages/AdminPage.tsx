    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[70vh] p-4">
        <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-100 max-w-sm w-full">
          <h2 className="text-xl font-bold text-slate-800 mb-6">관리자 로그인</h2>
          <form
            onSubmit={(e) => {
              e.preventDefault();
          ...
          >
            <div>
              <input
                ...
                className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-teal-500/20"
              />
  ...
  return (
    <div className="p-4 md:p-8 max-w-4xl mx-auto w-full">
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-slate-800 mb-2">관리자 설정</h1>
          <p className="text-slate-500">조직도 데이터를 관리하고 업데이트합니다.</p>
        </div>
        <button
      ...
      <div className="mt-8 bg-white rounded-2xl shadow-sm border border-slate-200 p-6 md:p-8">
        ...
        <div className="flex gap-4">
          <input
            ...
            value={useStore((state) => state.syncUrl)}
            onChange={(e) => useStore.getState().setSyncUrl(e.target.value)}
            className="flex-1 px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-teal-500/20"
          />
          <button 
            onClick={async () => {
